  ...
  _createClass(ReactForm, [{
    ...
    value: function _handleChange(elementName, e) {
      ...
      if (elemConf[elementName].type === 'checkbox') {
        ...
        this.setState({ elementsConf: elemConf });
        if (elemConf[elementName].type === 'radio' || elemConf[elementName].type === 'select' || elemConf[elementName].type === 'custom') {
          this.props.formConfig.action.validation(this._performValidation(elementName));
        }
      }
    ...
    key: '_changeCustomElement',
    value: function _changeCustomElement(elementName, value) {
      this._handleChange(elementName, value);
    }
  }, {
    key: '_generateCustomElement',
    value: function _generateCustomElement(elementName, conf, elementId, attrSpread) {
      var FieldComponent = conf.fieldComponent;
      var props = Object.assign({}, conf.props);
    ...
    value: function render() {
      ...
      var formElements = _.map(this.props.formConfig.order, function (elementName) {
        return _this9._generateElementHtml(elementName);
      });
      return React.createElement(
        'div',
    ...
    key: '_isDevEnv',
    value: function _isDevEnv() {
      return _.get(process, 'env.NODE_ENV') === 'development';
    }
  }, {
    key: '_generateSubNote',
    value: function _generateSubNote(subnote) {
      if (subnote) {
        return React.createElement(
          'div',
          { className: 'subnote' },
